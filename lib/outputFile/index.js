...
const { entityFileHeader, controllerFileHeader, serviceFileHeader, serviceImplFileHeader } = require('../../config/template');
...
const handleEntityFile = (distinctWorkStreamList, distinctTableList, jsonObj) => {
    ...
    const tableFieldList = jsonObj.map(x => {
        ...
        return {
            ...
            type: handleType((setting.dataTypeMapping.find(dt => dt.mapping.find(m => startWith(x["Data Type"], m))) || { type: x["Data Type"] }).type)
        };
    });
...
const handleFunctionFile = (distinctWorkStreamList, jsonObj) => {
    ...
    const functionList = jsonObj.map(x => {
        ...
        return {
            ...
            output: (setting.apiOutputMapping.find(a => a.mapping.find(m => startWith(upperCase(x["Output filed"]), upperCase(m)))) || { type: x["Output filed"] }).type,
            method: (setting.apiMethodMapping.find(a => a.mapping.find(m => startWith(x["Common Function name"], m))) || { type: x["Common Function name"] }).type,
...
const outputServiceImplFile = (distinctWorkStreamList, functionList) => {
    ...
    distinctWorkStreamList.forEach(workStream => {
        ...
        const functionStringList = functionList.filter(x => x.workStream === workStream).map(x => {
            ...
            const crossServiceTemplate = x.crossService.filter(x => x.split('').length > 0).map(x => {
                return `\t\t//Please be reminded that this function will have a cross service for: ${x}${os.EOL}`;
            }).join(os.EOL);

            const logicTemplate = x.logic.filter(x => x.split('').length > 0).map(x => {
                return `\t\t//TO-DO: ${x}${os.EOL}`;
            }).join(os.EOL);

            const subFunctionsTemplate = x.subFunctions.filter(x => x.split('').length > 0).map((y, i) => {
            ...
            const functionTemplate = `${transactionalTemplate}\tpublic ${x.output.replace('request:', '')} ${x.name}(${inputTemplate}) {${os.EOL}${crossServiceTemplate}${os.EOL}${subFunctionsTemplate}${os.EOL}${logicTemplate}\t}${os.EOL}`;
