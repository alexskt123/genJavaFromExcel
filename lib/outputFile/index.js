...
const listToArray = (field, curMode) => {
  return isSAD(curMode) && field.includes('List<') ? `${field.replace('List<', '').replace('>', '')}[]` : field;
};

const handleEntityFile = (distinctWorkStreamList, distinctTableList, jsonObj, curMode) => {
  try {
    ...
    const entityFiles = outputEntityFile(distinctWorkStreamList, distinctTableList, tableFieldList, curMode);
    return entityFiles;
  } catch (_e) {
    return [];
  }
...
const handleFunctionFile = (distinctWorkStreamList, jsonObj, mapperObj, curMode) => {
  try {
    distinctWorkStreamList.forEach((x) => createWorkStreamFolders(x));
    ...
    const controllerFile = outputControllerFile(distinctWorkStreamList, filteredFunctionList);
    const serviceFile = outputServiceFile(distinctWorkStreamList, filteredFunctionList, curMode);
    const serviceImplFile =
      (!isSAD(curMode) && outputServiceImplFile(distinctWorkStreamList, filteredFunctionList)) || [];
    const mapperFile =
      !isSAD(curMode) && mapperObj && outputMybatisFile(distinctWorkStreamList, filteredFunctionList, mapperObj);
...
const outputEntityFile = (distinctWorkStreamList, distinctTableList, tableFieldList, curMode) => {
  try {
    const fileList = [];
      ...
      distinctTableList
        ...
        .forEach((t) => {
          ...
          const fieldStringList = tableFieldList
            .filter((x) => x.table === t)
            .map((x) => {
              const columnString = isSAD(curMode) ? `\t@Column(name = "${x.column}")${os.EOL}` : '';
              const fieldString = `${columnString}\tprivate ${listToArray(getEntityType(x.type), curMode)} ${x.field};${
                os.EOL
              }`;
              return fieldString;
            })
          ...
          const extend = isSAD(curMode) ? ` extends BaseEntityUuid` : '';

          const fileContent = `${entityFileHeader[curMode]}${os.EOL}public class ${t}${extend} {${os.EOL}${fieldStringList}${os.EOL}} ${os.EOL}`;
...
const outputMybatisFile = (distinctWorkStreamList, functionList, mapperObj) => {
  try {
    const fileList = [];
    distinctWorkStreamList.forEach((workStream) => {
      const fileName = `${setting.outputPath}${workStream}/data/${titleCase(workStream)}.xml`;

      const mapper = functionList
        .filter((x) => x.workStream === workStream)
        .reduce(
          (acc, cur) => {
            const sqlList = mapperObj
              .filter((m) => m['Workstream'] === workStream && m['Common Function name'] === cur.name)
              .map((x) => {
                return {
                  [x.statementType]: [
                    {
            ...
            return {
              ...acc,
              mapper: [...acc.mapper, ...sqlList],
            };
          },
          { mapper: [] },
        );
      const fileContent = xmlFormatter(xml(mapper, { declaration: true }));
...
const outputServiceFile = (distinctWorkStreamList, functionList, curMode) => {
  try {
    const fileList = [];
    distinctWorkStreamList.forEach((workStream) => {
      ...
      const fileContent = `${fileHeader[curMode]}${functionStringList}${fileTailer}`;
